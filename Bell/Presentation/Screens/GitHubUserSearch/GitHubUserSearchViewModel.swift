...
final class GitHubUserSearchViewModel: ObservableObject {
    ...
    @Published private(set) var loadingState: GitHubUserLoadingState = .idle
    ...
    func onSubmitSearch() {
        ...
        self.gitHubUserController.listGitHubUser(input: .init(after: nil, first: 10, query: self.searchText))
            .sink { completion in
                switch completion {
                case .finished:
                    break
    ...
    func performAdditionalRequest() {
        ...
        self.updateState()
        self.gitHubUserController.listGitHubUser(input: .init(after: self.pageInfo?.endCursor, first: 20, query: self.searchedText))
            .sink { completion in
                switch completion {
                case .finished:
                    break
